...
def filtering(filter_type: str,key="default", iterator = None):
    ...
    match filter_type:
        case "main":
            ...
            st.session_state.filters = {"division": st.multiselect("חטיבה", RollDownOptions.divisions, default=default_val),
                                        "council": st.multiselect("מועצה", RollDownOptions.councils, default=default_val),
                                        "type": st.multiselect("סיווג", RollDownOptions.types, default=default_val),
                                        "distance": st.multiselect("מרחק מהגדר", RollDownOptions.distances, default=default_val)}
        case "settlements":
            all_shortcut = st.checkbox('הצג הכל',key=key)
            default_val = ['הכל'] if all_shortcut else None
            data_load_measure_state["load_stack"] += 1 if all_shortcut else 0
...
with st.expander("מבט כללי"):
    with st.form(key="general_view"):
        ...
        with col1:
            st.session_state.filters['division'] = st.multiselect("חטיבה", RollDownOptions.divisions)
            st.session_state.filters['council'] = st.multiselect("מועצה", RollDownOptions.councils)
            st.session_state.filters['type'] = st.multiselect("סיווג", RollDownOptions.types)
            st.session_state.filters['distance'] = st.multiselect("מרחק מהגדר", RollDownOptions.distances)
        ...
        with col2:
            ...
            for line in sheet:
                tab_line = line
                if ((tab_line['חטיבה'] in st.session_state.filters['division'] or _all in st.session_state.filters[
                    'division']) and
                        (tab_line['מועצה'] in st.session_state.filters['council'] or _all in st.session_state.filters[
                            'council']) and
                        (tab_line['סיווג'] in st.session_state.filters['type'] or _all in st.session_state.filters[
                            'type']) and
                        (tab_line['מרחק מהגדר'] in st.session_state.filters['distance'] or _all in
                         st.session_state.filters['distance'])):
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    'axis': {'range': [None, 100]},
                    'bar': {'color': "red" if st.session_state.all_avg < 50 else (
                        "yellow" if 50 < st.session_state.all_avg < 90 else "green")},
                    'steps': [
            ...
            text = "לא תקין" if st.session_state.all_avg < 50 else (
                "חלקי" if 50 < st.session_state.all_avg < 90 else "תקין")
            color = "error" if st.session_state.all_avg < 50 else (
                "part" if 50 < st.session_state.all_avg < 90 else "success")
        ...
        with col4:
            ...
            for line in sheet:
                if ((line['חטיבה'] in st.session_state.filters['division'] or _all in st.session_state.filters[
                    'division']) and (
                        line['מועצה'] in st.session_state.filters['council'] or _all in st.session_state.filters[
                    'council']) and (
                        line['סיווג'] in st.session_state.filters['type'] or _all in st.session_state.filters[
                    'type']) and (
                        line['מרחק מהגדר'] in st.session_state.filters['distance'] or _all in st.session_state.filters[
                    'distance'])):
                    st.session_state.data_pi["כמות"][
                        st.session_state.data_pi["סטטוס כולל"].index(line["סטטוס כולל"])] += 1
...
with st.expander("חיתוך נתונים רב מימדי"):
    ...
    if chosen_view_option == options_translator["scores"]:
        ...
        with st.form(key="main details", width="stretch"):
            ...
            rows = 0
            cols = 0
            log.debug(log_pref(locations=_locations, message="building the present matrix"))
            ...
            for line in sheet:
                ...
                if is_passing_basic_filter():
                    ...
                    with grid[rows][cols]:
                        badge_text = "לא תקין" if val < 50 else ("חלקי" if 50 < val < 90 else "תקין")
                        badge_color = "error" if val < 50 else ("part" if 50 < val < 90 else "success")
                        make_linked_gauge_labeled_component(tab_line["ישוב"], val, tab_line["ישוב"], badge_text,
                                                            badge_color)
    ...
    # wide
        ...
        log.debug(log_pref(locations=_locations, message="starting tb segment"))
        ...
        if sub_option_wide == _raw:
            ...
            log.debug(log_pref(locations=_locations, message="starting stb segment"))
            with st.form(key="data_analysis", width="stretch"):
                ...
                log.debug(log_pref(locations=_locations, message="tighten search scope with main filters"))
            ...
            log.debug(log_pref(locations=_locations, message=f"len of main_raw_data_frame is {len(st.session_state.main_raw_data_frame)}"))
            with st.spinner("מושך נתונים ..."):
                ...
                with ThreadPoolExecutor(max_workers=67) as executor:
                    ...
                    for future in as_completed(future_to_sheet):
                        ...
                        log.debug(log_pref(locations=_locations, message=f"working grabbing data for {settlement_name=}"))
                        try:
                            # data is a list of lists of the settlement
                            settlement_data = future.result()
                            log.debug(log_pref(locations=_locations,message=f"received data for {settlement_name=} "))
                            ...
                            key_transletor = {"status":'סטטוס',"action":'הפעלה',"types":'סיווג',"frame":'מסגרת',"domain":'תחום',"model":'שיוך למודל',"economy":'משק',"family":'משפחה'}
            ...
            #         st.session_state.main_raw_data_frame.pop(item)
                ...
                if True:
                    ...
                    for criteria, total_sum in  st.session_state.avarages_data_set.items():
                        ...
                        log.debug(log_pref(locations=_locations, message=f"got {st.session_state.avarages_data_set[criteria]=}"))

                if st.session_state.data_view_options_raw == "שעונים":
                    log.debug(log_pref(locations=_locations,message=" clocks option elected"))
                    _locations["segment"] = "clocks option logic"
                    log.debug(log_pref(locations=_locations, message=f"working on data: {[(criteria, val) for criteria, val in st.session_state.main_raw_data_frame.items()]}"))
